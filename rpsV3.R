#V3
#Grupo: Adrian Lozano, Jose, Javier, Angel
...
obtener_mejor_contra_juego_perfil <- function(perfil) {
  eleccion_contrajuego <- switch(
    ...
    r = "p",  # Si el usuario elige más Piedra, elige Papel
    p = "s",  # Si el usuario elige más Papel, elige Tijera
    s = "r",  # Si el usuario elige más Tijera, elige Piedra
    "r"  # Valor por defecto para cualquier otra situación
  )
...
actualizar_historial_v3 <- function(jugador_identificador, eleccion_usuario, eleccion_bot, resultado) {
  if (!file.exists("historial_detalle.csv")) {
    # Creamos un data frame vacío con las mismas columnas que 'nueva_entrada'
...
jugar_piedra_papel_tijeras_v3 <- function() {
  ...
  cat("Introduzca piedra (R), papel (P) o tijera (S), Q para salir y h para mostrar el historial\n")
